    ...
    match &cli.command {
        ...
        Commands::AddSF2 { input_glob, output_folder, swdl: swdl_path, out_swdl: out_swdl_path, resample_threshold, sample_rate, sample_rate_adjustment_curve, adpcm_encoder_lookahead, pitch_adjust, midi_specific_optimization } => {
            ...
            for (input_file_path, output_file_path) in input_file_paths {
                ...
                fn copy_raw_sample_data<R>(mut sf2file: R, sf2: &SoundFont2, bank: &mut SWDL, dsp_options: DSPOptions, sample_rate_adjustment_curve: usize, pitch_adjust: i64, filter_samples: fn(&(usize, &SampleHeader)) -> bool) -> Result<(usize, Vec<SampleInfo>), DSEError>
                where
                    R: Read + Seek {
                ...
                fn copy_presets(sf2: &SoundFont2, sample_infos: &mut Vec<SampleInfo>, first_available_id: usize, sample_rate_adjustment_curve: usize, pitch_adjust: i64) -> PointerTable<ProgramInfo> {
